...
async function main() {
  ...
  console.log(`- S3 Bucket: ${deploymentBucket.name} (${deploymentBucket.endpoint})`);
  console.log(`- Admin Panel: ${adminPanel.bucket.name} (${adminPanel.deployment.filesUploaded} files)`);
  console.log(`  - URL: ${adminPanel.ingress.url}`);
  console.log(`- Landing Page: ${landingPage.bucket.name} (${landingPage.deployment.filesUploaded} files)`);
