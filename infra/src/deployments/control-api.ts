 ...
 */
export async function deployControlApi(props: ControlApiDeploymentProps): Promise<ControlApiDeploymentResult> {
  ...
  const deployment = await Deployment('control-api', {
    ...
    env: [
      ...
      // Database connection
      { name: 'DATABASE_URL', value: `postgresql://origan_root:${process.env.POSTGRES_PASSWORD || 'postgres'}@${props.databaseEndpoint}/origan` },
      ...
      { name: 'GITHUB_APP_PRIVATE_KEY_BASE64', value: process.env.GITHUB_APP_PRIVATE_KEY_BASE64 },
      { name: 'GITHUB_WEBHOOK_SECRET', value: process.env.GITHUB_WEBHOOK_SECRET },
      
      // Docker Registry and build configuration
      { name: 'DOCKER_REGISTRY', value: 'docker-registry.platform.svc.cluster.local:5000' },
      { name: 'BUILD_RUNNER_IMAGE', value: 'registry.platform.origan.dev/runner:latest' }
    ],
    resources: {
      requests: {
        memory: '256Mi',
