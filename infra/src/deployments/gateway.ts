 ...
 */
export async function deployGateway(props: GatewayDeploymentProps): Promise<GatewayDeploymentResult> {
  ...
  const deployment = await Deployment('gateway', {
    ...
    env: [
      ...
      { name: 'CONTROL_API_URL', value: 'http://control-api.origan.svc.cluster.local:9999' },
      { name: 'RUNNER_URL', value: 'http://runner.origan.svc.cluster.local' },
