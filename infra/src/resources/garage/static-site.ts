 ...
 */
export interface GarageStaticSite extends Resource<"garage::StaticSite">, GarageStaticSiteProps {
  /**
   * Website URL
   */
  websiteUrl: string;
 ...
 * @example
 * // Deploy admin panel
 * const adminPanel = await GarageStaticSite("admin-panel-deployment", {
...
export const GarageStaticSite = Resource(
  "garage::StaticSite",
  async function(this: Context<GarageStaticSite>, name: string, props: GarageStaticSiteProps): Promise<GarageStaticSite> {
    ...
    console.log(`Deploying static site ${name} from ${props.sourceDir} to ${props.bucketName}...`);
    ...
    if (props.cleanupOrphaned && existingFiles.size > 0) {
      const orphanedFiles = Array.from(existingFiles).filter(f => !localFiles.has(f));
    ...
    if (uploadErrors.length > 0) {
      console.warn(`Failed to upload ${uploadErrors.length} files: ${uploadErrors.join(', ')}`);
    }
