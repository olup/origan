 ...
 */
export interface GarageBucket extends Resource<"garage::Bucket">, GarageBucketProps {
  /**
   * Bucket name
   */
  name: string;
 ...
 * const websiteBucket = await GarageBucket("admin-panel", {
...
export const GarageBucket = Resource(
  "garage::Bucket",
  async function(this: Context<GarageBucket>, name: string, props: GarageBucketProps = {}): Promise<GarageBucket> {
    const host = process.env.K3S_SSH_HOST || '62.171.156.174';
    const endpoint = props.endpoint || process.env.GARAGE_ENDPOINT || 'https://s3.platform.origan.dev';
    ...
    if (this.phase === "delete") {
      try {
        ...
        console.log(`Bucket ${name} permissions revoked. Bucket not deleted to preserve data.`);
      } catch (error) {
