 ...
 */
export interface NatsDeployment extends Resource<"nats::Deployment">, NatsDeploymentProps {
  /**
   * Service endpoint
   */
  endpoint: string;
...
export const NatsDeployment = Resource(
  "nats::Deployment",
  async function(this: Context<NatsDeployment>, name: string, props: NatsDeploymentProps = {}): Promise<NatsDeployment> {
...
` : ''}
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (readyReplicas === desiredReplicas) {
          console.log(`✅ NATS StatefulSet ${name} is ready (${readyReplicas}/${desiredReplicas} replicas)`);
          break;
        }
    ...
    if (retries === maxRetries) {
      throw new Error(`Timeout waiting for NATS StatefulSet ${name} to be ready`);
    }
