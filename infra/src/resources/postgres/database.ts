 ...
 */
export interface PostgresDatabase extends Resource<"postgres::Database">, PostgresDatabaseProps {
  /**
   * Service endpoint
   */
  endpoint: string;
...
export const PostgresDatabase = Resource(
  "postgres::Database",
  async function(this: Context<PostgresDatabase>, name: string, props: PostgresDatabaseProps): Promise<PostgresDatabase> {
        ...
        template: {
          ...
          spec: {
            containers: [{
              ...
              livenessProbe: {
                exec: {
                  command: ['pg_isready', '-U', props.user, '-d', props.database]
                },
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (readyReplicas === desiredReplicas) {
          console.log(`✅ PostgreSQL StatefulSet ${name} is ready (${readyReplicas}/${desiredReplicas} replicas)`);
          break;
        }
        
        console.log(`⏳ Waiting... (${readyReplicas}/${desiredReplicas} replicas ready)`);
      } catch (error) {
    ...
    if (retries === maxRetries) {
      throw new Error(`Timeout waiting for PostgreSQL StatefulSet ${name} to be ready`);
    }
