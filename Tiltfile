...
def build_with_reload(name):
    ...
    if os.path.exists("./packages/{}/package.json".format(name)):
        live_update.append(run("pnpm install", trigger=["package.json", "pnpm-lock.yaml"]))
...
local_resource("cli", cmd="cd packages/cli && pnpm build", deps="packages/cli", ignore=["packages/cli/dist"], labels=["1-main"])
