...
export class S3DirectorySync extends pulumi.ComponentResource {
  ...
  constructor(name: string, args: S3DirectorySyncArgs, opts?: pulumi.ComponentResourceOptions) {
    ...
    const env = pulumi.all([args.accessKey, args.secretKey]).apply(([accessKey, secretKey]) => ({
    ...
    const syncCommand = new command.local.Command(`${name}-sync`, {
